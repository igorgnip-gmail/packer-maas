...
Separately, the kickstart script itself creates a local ```rockylinux``` account (passwordless sudo, key-only SSH -- the default sshd configuration disallows password-based authentication). Its password is not baked into the template: set the `TEMPLATE_USER_PASSWORD_HASH` environment variable to a SHA-512 crypt hash before running `packer build` if you want this account to also have a working local/console password; otherwise it stays locked and SSH-key-only.
