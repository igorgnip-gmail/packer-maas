...
# as rocky9/alma9/ol9's own kickstarts).
user --groups=wheel --name=oraclelinux --gecos="Oracle Linux"

repo --name="ol10_AppStream" ${KS_APPSTREAM_REPOS} ${KS_PROXY}
...
# so this same %packages list works on both amd64 (grub2-efi-x64,
# shim-x64) and arm64 (grub2-efi-aa64, shim-aa64) without an arch
# conditional -- same approach rocky9's kickstart already uses. UNTESTED
# on OL10 specifically (see ol10.pkr.hcl's OPEN QUESTIONS block).
